#!/usr/bin/env python3
#
# VENDORED from https://github.com/KaibinBao/healforcescpecg  (MIT License,
# Copyright (c) 2019 Kaibin Bao — see NOTICE for the full license text).
#
# Local modification (marked "NUMPY2 PATCH" below): the bit-masking in
# _decompress_healforce_coding() is done in uint16 to avoid an OverflowError
# under numpy >= 2.0. No decoding logic was changed.
#
...
class HealforceSCPECG:
    ...
    def _decompress_healforce_coding(self):
        ...
        # NUMPY2 PATCH: do bit masking in uint16 to avoid int16 overflow
        u = samples.astype(np.uint16)
        self.beats = (u & 0x8000) >> 15
        # irregular heart beat detected by hardware - details are in section 9
        self.marked_beats = (u & 0x4000) >> 14
        ...
        samples = (((((u & 0x0300) >> 8) + 0xFE) & 0xFF) << 8) + (u & 0xFF)
        # samples are quantized in 0.01 mV steps, i.e. 100 ~= 1 mV
        self.samples = samples.astype(np.uint16).view(np.int16)
